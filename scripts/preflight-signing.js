...
const storePath = path.isAbsolute(storeFile) ? storeFile : path.join(androidDir, 'app', storeFile);
...
if (keyPassword !== storePassword) {
  fail(
    'keyPassword: PKCS12 keystores have no separate key password — keyPassword must equal storePassword (set ANDROID_KEY_PASSWORD to ANDROID_KEYSTORE_PASSWORD)',
  );
}
