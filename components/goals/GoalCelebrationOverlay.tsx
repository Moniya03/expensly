...
export function GoalCelebrationOverlay({ visible, goal, onDone }: Props) {
  ...
  const appreciation = useMemo(() => appreciationLines[Math.floor(Math.random() * appreciationLines.length)], [goal?.id]);
              ...
              <Animated.Text
                ...
                style={[
                  styles.sparkle,
                  {
                    left: sparkle.left,
                    top: sparkle.startY,
                    opacity: value.interpolate({ inputRange: [0, 0.3, 1], outputRange: [0, sparkle.opacity, 0] }),
                    transform: [
                      {
                        translateY: value.interpolate({ inputRange: [0, 1], outputRange: [0, -screen.height * 0.55] }),
                      },
                      {
                        translateX: value.interpolate({ inputRange: [0, 1], outputRange: [0, sparkle.drift] }),
                      },
                      {
                        scale: value.interpolate({ inputRange: [0, 0.5, 1], outputRange: [0.7, 1, 0.9] }),
                      },
                    ],
                  },
                ]}
...
const createStyles = (colors: Colors) => StyleSheet.create({
  ...
  goalName: { color: colors.onSurface, fontSize: typography.fontSize.xl, fontWeight: typography.fontWeight.bold },
  goalDeadline: { color: colors.onSurfaceVariant, fontSize: typography.fontSize.sm },
  goalPercent: { color: colors.secondary, fontSize: typography.fontSize.lg, fontWeight: typography.fontWeight.bold },
  progressTrack: { height: 8, borderRadius: 999, backgroundColor: colors.surfaceContainer, overflow: 'hidden' },
  progressFill: { width: '100%', height: '100%', backgroundColor: colors.secondary },
  ...
  appreciationLabel: { color: colors.onSurfaceVariant, fontSize: typography.fontSize.xs, textTransform: 'uppercase', letterSpacing: 0.7 },
  appreciationText: { color: colors.onSurface, fontSize: typography.fontSize.md, lineHeight: 22, fontWeight: typography.fontWeight.medium },
  doneButton: {
    minHeight: 50,
    borderRadius: borderRadius.lg,
  ...
  doneText: { color: colors.surface, fontSize: typography.fontSize.md, fontWeight: typography.fontWeight.semiBold },
});
